...
exports.startSmartAlertsJob = void 0;
...
async function sendAlertEmail(email, subject, html) {
    try {
        ...
        const transporter = nodemailer_1.default.createTransport({ host, port, auth: { user, pass } });
    ...
    node_cron_1.default.schedule('0 0 * * *', async () => {
        ...
        try {
            ...
            for (const alloc of overdueAllocations) {
                ...
                if (!existingNotif) {
                    // Notify Employee
                    const empNotif = await Notification_1.Notification.create({
                        receiver: employee._id,
                        title,
                        message,
                    ...
                    socketService_1.socketService.emitToUser(employee._id.toString(), 'notification', empNotif);
                    // Email Employee
                    if (employee.email) {
                        await sendAlertEmail(employee.email, `AssetFlow Alert: Overdue Return - ${asset.name}`, `<p>Hello ${employee.firstName},</p>
               ...
               <p>Please return the equipment to your department coordinator as soon as possible.</p>`);
                    ...
                    for (const mgr of managers) {
                        const mgrNotif = await Notification_1.Notification.create({
                            receiver: mgr._id,
                        ...
                        socketService_1.socketService.emitToUser(mgr._id.toString(), 'notification', mgrNotif);
                    }
                }
            }
            // ----------------------------------------------------
            ...
            const expiringAssets = await Asset_1.Asset.find({
                deletedAt: null,
            ...
            const idleAssets = await Asset_1.Asset.find({
            ...
            const repairFreq = await MaintenanceRequest_1.MaintenanceRequest.aggregate([
                { $match: { createdAt: { $gte: sixMonthsAgo } } },
                { $group: { _id: '$asset', count: { $sum: 1 } } },
                { $match: { count: { $gt: 3 } } }
            ]);
            for (const record of repairFreq) {
                const asset = await Asset_1.Asset.findById(record._id);
                if (!asset)
                    continue;
...
exports.startSmartAlertsJob = startSmartAlertsJob;
